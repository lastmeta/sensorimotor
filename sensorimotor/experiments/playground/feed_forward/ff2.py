'''
forward both ways with different weights, allows for different sized layers
'''
import numpy as np
...
class NeuralNetwork:
    ...
    def train(self, X, y, learning_rate=0.5):
        ...
        self.h_input_weights += self.h_i_delta_w
        self.h_output_weights += self.h_o_delta_w
    ...
    def forward(self, X):
        ...
        h_output_activation = np.dot(i_hidden_output, self.h_output_weights)
    ...
    def backwards(self, y):
        ...
        h_input_activation = np.dot(o_hidden_output, self.h_input_weights)
